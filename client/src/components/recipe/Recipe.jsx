...
const Recipe = () => {
    ...
    const checkIfRecipeIsLiked = useCallback(
      async (recipeId) => {
          ...
          try {
              ...
              if (!response.ok) {
                  console.error("checkIfRecipeIsLiked: Failed to check if recipe is liked."); // Add this
  ...
  const handleLikeRecipe = async () => {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`Failed to update like status.  Status: ${response.status}`);
        }
      ...
      <body>
        <div class="print-container">
          ...
          ${recipe.allergyWarnings && recipe.allergyWarnings.length
            ...
            <ul>
              ${recipe.allergyWarnings.map((warn) => `<li>${warn}</li>`).join("")}
            </ul>
          `
                : ""
            }
            ...
            <div className="recipe-left-column">
                ...
                <div className="ingredients-section">
                        ...
                        {recipe?.ingredients?.map((ingredient, index) => (
                            <li key={index}>
                                ...
                                {ingredientAlternatives[ingredient.name] && (
                                    <p
                                        className={`alternative ${selectedIngredients.includes(ingredient.name)
                                            ? "highlighted"
                                            : ""
                                            }`}
                                    >
                    ...
                    {showIngredientSelection && (
                        <div
                            className={`ingredient-selection ${showIngredientSelection ? "" : "hidden"
                                }`}
                        >
