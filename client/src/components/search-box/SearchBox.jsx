...
function SearchBox({ searchTerm, searchInputRef, searchInputWrapperWidth, setRecipes }) {
  ...
  const handleItemClick = (itemName) => {
    setSelectedIngredients((prev) => {
      const updatedIngredients = prev.includes(itemName)
  ...
  const ingredients = useMemo(
    () => [
      {
        name: "Onion",
        image: "https://gyanagri.com/wp-content/uploads/2021/03/onion-images.jpg",
      },
      {
        name: "Garlic",
        image:
      ...
      {
        name: "Cheese",
        image: "https://c8.alamy.com/comp/DEJ9F6/swiss-cheese-isolated-on-a-white-background-DEJ9F6.jpg",
      },
      {
        name: "Chicken",
        image:
    ...
    <div className="searchbox-container" style={{ position: "relative" }}>
      {isDropdownOpen &&
        ReactDOM.createPortal(
          <div
            ref={dropdownRef}
            onMouseDown={(e) => e.stopPropagation()}  // Prevent any mousedown inside dropdown from bubbling up
          ...
          >
            {filteredIngredients.length > 0 ? (
              ...
              >
                ...
                >
                  Show Recipes ({selectedIngredients.length})
                </button>
              </div>
            ) : (
              <div
